...
class ClassDocInfo
{
	...
	public static function getFieldInfo($className)
	{
		if(class_exists($className)){
			$info = getArrVal( $className ,self::$fieldCache);
			if ( !$info ) {
				$info = self::getClassInfo( $className );
				self::$fieldCache[$className] = $info;
			}

			return getArrVal('property',$info);
		}
	}
		...
		if($properties){
			foreach ( $properties as $property ) {
				if($property->isPublic()){
					...
					$hasEntity = getArrVal( 'entity' , $field[$name] );
					if($hasEntity){
	...
	/**
	 * @return null|\AtServer\DocParser
	 */
